...
def jlink_get_symbol_inside_data_idx(cline, sstart, send, idx):
    if cline.find(sstart) == -1:
        return;
    cursplit = cline;
    target_idx=0;
    while idx > 0:
        target_idx = cline.find(sstart, target_idx+1)
        print cline[target_idx:];
        idx = idx - 1;
    cursplit = cline[target_idx:];
    if cursplit.find(send) != -1:
        return cursplit[1:cursplit.find(send)]
    return ""
...
for cur_line in pt_lines:
    ...
    print cur_list
    pt_name = jlink_get_symbol_inside_data_idx(cur_line, "(", ")", 1);
    st_addr = int(jlink_get_symbol_inside_data_idx(cur_line, "(", ")", 3), 16);
    size = int(jlink_get_symbol_inside_data_idx(cur_line, "(", ")", 4), 16);
