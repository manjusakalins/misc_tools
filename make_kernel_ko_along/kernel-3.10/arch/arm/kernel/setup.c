...
extern unsigned char * get_hw_info(void);
int to_mod_cpu = 0;
EXPORT_SYMBOL(to_mod_cpu);
int plat_cpu = 0;
char *fplatform[] = {
    "DEF_PLAT",
    "MT6577",
...
static int __init topology_init(void)
{
    ...
    if (to_mod_cpu == 1)//fix for old cpu cores
        to_mod_cpu = 2*nr_cpu_ids;
    if (to_mod_cpu == 2)//fix for old cpu cores
        to_mod_cpu = 4*nr_cpu_ids;
	...
	if (to_mod_cpu) {
        int cpunum=nr_cpu_ids;
        for (; cpunum < to_mod_cpu; ) {
            for_each_possible_cpu(cpu) {
                if (cpunum > nr_cpu_ids && cpunum >= to_mod_cpu)
                    break;
                struct cpu *cp = kzalloc(sizeof(*cp), GFP_KERNEL);
                if (!cp)
                    return -ENOMEM;
                cp->hotpluggable = 0;
                register_cpu(cp, cpunum);
                cpunum++;
            }
        }
    }
    ...
    if (to_mod_cpu) {
        ...
        for (; cpu < to_mod_cpu; ) {
            for_each_online_cpu(i) {
                if (cpu+i  > nr_cpu_ids && cpu+i >= to_mod_cpu)
                    break;
                /*
