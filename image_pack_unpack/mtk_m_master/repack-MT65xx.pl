...
if ( $ARGV[0] eq "-boot" || $ARGV[0] eq "-recovery" ) {
	...
	die "Usage: $usage" unless $ARGV[2] && $ARGV[3] && !$ARGV[4];
	repack_custom();
} elsif ( $ARGV[0] eq "-userdata" ) {
	die "Usage: $usage" unless $ARGV[2] && $ARGV[3] && !$ARGV[4];
	repack_userdata();
}else {
	die "Usage: $usage";
}
#6735 make_ext4fs -s -T -1 -S out/target/S960/root/file_contexts -l 1610612736 -a system system.img syste/
sub repack_system {
	print "./make_ext4fs.ext4fs -s -T -1 -S file_contexts -l $ARGV[1] -a system $ARGV[3] $ARGV[2]\n";
	if (-e "$ARGV[3]")
	{
	    system ("rm -rf $ARGV[3]");
	}
	if (-e "$ARGV[2]")
	{	   
	   system ("./make_ext4fs.ext4fs -s -T -1 -S file_contexts -l $ARGV[1] -a system $ARGV[3] $ARGV[2]");
	}else{	   
	   die "NO $ARGV[2] directory.";	
	}
	print "completed!!!\n";
}

sub repack_custom {
	print "./make_ext4fs.ext4fs -s -T -1 -S file_contexts -l $ARGV[1] -a custom $ARGV[3] $ARGV[2]\n";
	if (-e "$ARGV[3]")
	{
	    system ("rm -rf $ARGV[3]");
	}
	if (-e "$ARGV[2]")
	{	   
	   system ("./make_ext4fs.ext4fs -s -T -1 -S file_contexts -l $ARGV[1] -a custom $ARGV[3] $ARGV[2]");
	}else{	   
	   die "NO $ARGV[2] directory.";	
	}
	print "completed!!!\n";
...
sub repack_userdata {
	print "./make_ext4fs.ext4fs -s -T -1 -S file_contexts  -l $ARGV[1] -a data $ARGV[3] $ARGV[2]\n";
	if (-e "$ARGV[3]")
	{
	    system ("rm -rf $ARGV[3]");
	}
	if (-e "$ARGV[2]")
	{	   
	   system ("./make_ext4fs.ext4fs -s -T -1 -S file_contexts -l $ARGV[1] -a data $ARGV[3] $ARGV[2]");
	}else{	   
	   die "NO $ARGV[2] directory.";	
	}
	print "completed!!!\n";
