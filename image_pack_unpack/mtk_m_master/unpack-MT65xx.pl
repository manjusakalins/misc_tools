...
if ((substr($input, 0, 4) eq "\x88\x16\x88\x58") & (substr($input, 8, 4) eq "LOGO")) {
	...
	if( $ARGV[1]){
		...
		$ARGV[1] =~ s/-//;
		$ARGV[1] =~ s/_only//;
		unpack_system($input,$ARGV[1]);
	} else {
		unpack_system($input,"system");
	}
}else {
...
sub unpack_system{
	if (-e "_ext4_$ARGV[0]"){
	   system ("rm -rf _ext4_$ARGV[0].img");
	}

	system ("./simg2img $ARGV[0] _ext4_$ARGV[0]");
	if (-b "$ARGV[0]-images")
	{
	   system ("umount $ARGV[0]-images");	
	} else {
	   if (-e "$ARGV[0]-images")
	   {
		system ("rm -rf $ARGV[0]-images/*");
	   }else{	
	   	mkdir "$ARGV[0]-images" or die;
	   }
	}	

	system ("mount -t ext4 _ext4_$ARGV[0] $ARGV[0]-images");

	print "Extracted ramdisk contents to directory '$ARGV[0]-images'\n";
}
